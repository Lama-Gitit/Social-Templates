// ============================================================
// RandomGeneratorModalV2.tsx
// Version: 2.0.0
// Changes from V1:
//   - Fix: updateUsage slice(-1) → slice(-2) so 2/min rate limit works
//   - Fix: Remove double-sanitization of history SVGs on render
//   - Fix: Replace deprecated substr() with substring()
//   - Perf: Trim history context in prompt to last 3 labels (was all 20)
//   - Cost: Switched generate-layout to Haiku via AI_MODEL env override
//     (see _provider.ts — set LAYOUT_MODEL=claude-haiku-4-5-20251001)
// ============================================================
...
export function RandomGeneratorModal({ isOpen, onClose }: RandomGeneratorModalProps) {
    ...
    // Sanitize results only when they change — no double-pass needed
    const sanitizedResults = useMemo(
        () => generatedResults.map(r => ({ ...r, safeSvg: sanitizeSVG(r.svg) })),
        [generatedResults]
    );

    // FIX: History is already sanitized on load — skip re-sanitizing here
    // V1 was sanitizing twice: once on localStorage load, once in this memo
    const sanitizedHistory = useMemo(
        () => history.map(r => ({ ...r, safeSvg: r.svg })),
        [history]
    );
    ...
    useEffect(() => {
        ...
        // Load History — sanitize SVGs once here to prevent stored XSS
        const savedHistory = localStorage.getItem('generator_history');
        if (savedHistory) {
            try {
                const parsed = JSON.parse(savedHistory);
                const cleaned = Array.isArray(parsed)
                    ? parsed.map((item: { id: string; label: string; svg: string; platform: string; date: string }) => ({
                        ...item,
    ...
    const updateUsage = () => {
        const now = Date.now();
        const newUsage = {
            // FIX: Was slice(-1) — only stored 1 timestamp, so 2/min limit never fired
            // Now slice(-2) correctly retains last 2 timestamps for rate check
            lastTimes: [...usage.lastTimes.slice(-2), now],
    ...
    const handleGenerate = async () => {
        ...
        // 2 Per Minute — FIX: now works correctly with slice(-2) above
        ...
        try {
            // PERF: Only pass last 3 history labels — was passing all 20, wasting tokens
            const recentLabels = history.slice(0, 3).map(entry => entry.label).join(", ");
...
- Use a maximum of 15 SVG elements per template.

PREVIOUS STYLES (DO NOT REPEAT): ${recentLabels.length > 0 ? recentLabels : "None yet"}.
